{ config, pkgs, ... }: {
  # we use the standard nixpkgs niri, no flake needed
  home.packages = [ pkgs.niri ];

  # we write the config file directly as a string
...
binds {
    ...
    SUPER+GRAVE { spawn-sh "~/.config/rofi/scripts/wallpaper-rofi-2.sh"; };
    ...
    SUPER+F5 { spawn-sh "gpu-screen-recorder -w screen -f 60 -fm cfr -k h264 -c mp4 -a $(pactl get-default-sink).monitor -o ~/Videos/$(date +%d.%m.%Y_%I:%M%p).mp4 & notify-send 'recording started'"; };
    SUPER+F4 { spawn-sh "pkill -INT -f gpu-screen-recorder && notify-send 'recording stopped'"; };
